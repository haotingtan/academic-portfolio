...
def loans_test():
    ...
    # added tests
    assert loans.Applicant("<20", []).lower_age() == 20
    assert loans.Applicant("760-740", []).lower_age() == 760
    assert loans.Applicant(">1", []).lower_age() == 1
    loans_points += 2
    ...
    #added tests
    applicants = sorted([
        loans.Applicant(">1", ["43", "44"]),
        loans.Applicant("40-50", ["1", "3"]),
        loans.Applicant("100-110", ["22"]),
        loans.Applicant("<45", ["5"]),
    ])
    assert [a.lower_age() for a in applicants] == [1, 40, 45, 100]
    loans_points += 2
